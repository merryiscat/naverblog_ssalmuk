...
def main() -> None:
    ...
    if login_ok:
        step_manual(
            state, "p2_ai_consent",
            "4단계 — 공개·검색 허용 확인 (P2) ※ 인용 대상이 되기 위한 실질 조건",
            "https://admin.blog.naver.com",
            "블로그 관리 > 기본 설정 > '공개 설정'에서 다음을 확인하세요:\n"
            "  ① 블로그와 글이 '전체공개'  ② '검색 허용(검색 노출)'이 켜져 있음\n"
            "※ 'AI 활용 동의'라는 별도 메뉴는 블로그 관리에 없습니다 (2026-08-17 확인).\n"
            "   동의는 네이버 약관 차원에서 처리되며, 로그인 중 AI 관련 동의 팝업이\n"
            "   뜨면 동의하면 됩니다. (에디터의 'AI 활용' 버튼은 AI 생성물 표시용 — 다른 기능)",
        )
        step_manual(
            state, "p7_profile",
