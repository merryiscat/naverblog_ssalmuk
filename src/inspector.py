...
REGULAR_CHECKS = [
    ...
    "출처 정밀도: (1) 각주 번호([1] 등)가 실링크 없이 원시 텍스트로 남아 있지 않은가 "
    "(2) 본문 출처가 도메인만 뭉뚱그리거나 여러 주장에 하나로 몰려 붙지 않고, 매체·기관명으로 "
    "구체적이며 클릭 가능한 링크로 검증 가능한가 — 단지 '(출처: …)'가 있다는 것만으로 pass하지 말고 "
    "'그 주장의 근거를 독자가 바로 확인할 수 있는가'로 판정하라",
    ...
    "이미지 관련성: 대표 이미지가 있다면 글 주제와 한눈에 연결되는가 — 주제와 무관한 "
    "추상 도형·벽지나 엉뚱한 소재(글은 정책인데 풍경·음식 등)면 fail. 이미지가 없는 것은 fail 아님",
...
**근거 없는 처방 금지 (2026-08-31, 헛일 재발 방지)**: 관리자 설정·플랫폼 기능에 기대는
개선(위젯 추가·스킨·고정 등)은 화면에서 **분명히 빠졌거나 깨진 것**이 아니면 fix로 단정하지
마라 — 이미 켜져 있거나(네이버는 카테고리·프로필 위젯이 기본 노출) 네이버가 지원하지 않는
기능('인기글·대표글 위젯'은 네이버에 없음)일 수 있다. 화면에서 검증 못 하는 것은 지시가 아니라
note에 '확인 필요'로만 남겨라. '운영신호 약함→위젯 켜라'류 일반론 처방이 3회 반복돼 사람에게
헛일을 시킨 전례가 있다(실제로는 이미 정상). 증상은 적되 처방은 화면 근거가 있는 것만.
