...
#    2026-09-04 점진 상향 4→6: 같은 니치 경쟁 블로그(blessed_pulse)가 8/일 발행·생존 실증.
#    단 발행 시각은 11~21시 창을 건수로 균등 분할(_random_publish_times)하므로 6건이면 간격이
#    ~1.67h로 좁아진다 → 우선 실선정을 5건(간격 정확히 2h)으로 두고, 저품질 신호 2주 관찰 후 6→8.
DAILY_PUBLISH_LIMIT = 6

# ①-b 하루 기본 선정(생성) 건수 — 오케스트레이터가 이 수만큼 selected를 고른다.
#    게이트 탈락으로 구멍이 나면 예비(reserve)를 자동 투입해 이 수를 지향한다.
#    2026-09-04 3→5 상향(발행량 확대). 발행 상한(6)보다 낮게 둬 간격 ~2h 유지, 예비가 뚜껑까지 보충.
DAILY_SELECT_COUNT = 5
...
#    2026-09-04 2→3 (발행량 확대에 맞춰 최소 목표도 상향).
DAILY_PUBLISH_TARGET = 3

# ①-e 생성 사이클 총 시도 상한 — selected 5 + reserve 2 = 7. 무한 보충 방지.
GENERATE_MAX_ATTEMPTS = 7
...
# 운영 대시보드 서빙 포트 (2026-09-04) — 미니PC LAN에서 브라우저 접속.
# Odin(8000)·tokbiseo와 안 겹치게. http://<서버IP>:8765/
DASHBOARD_PORT = 8765
