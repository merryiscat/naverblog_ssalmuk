...
DEFAULT_SEEDS_BY_CATEGORY = {
    ...
    "스타일": ["여름코디", "패션기초"],
    "테크": ["노트북추천", "스마트홈"],
    "라이프": ["정부지원금", "청년지원금", "육아지원금", "부모급여", "아동수당",
              "긴급복지지원", "에너지바우처", "문화누리카드", "소상공인지원"],
    "컬쳐": ["전시회추천", "독서모임"],
    "미디어": ["넷플릭스추천", "드라마정보"],
    "인사이트": ["근로장려금", "자녀장려금", "기초연금", "국민취업지원제도",
                "청년월세지원", "주거급여", "고용지원금"],
    "취미": ["캠핑초보", "홈트레이닝"],
}
...
# 참조 레퍼런스 블로그 — '일반인이 받는 보조금·지원금'을 내는 부처 공식 블로그.
# 2026-09-08 교체: 기존 mcst_pr(문체부 K-공감)은 문화·체육·관광 위주라 영화·팔만대장경·전시·
# 수목원·KTX 같은 비수급 잡음을 쏟아냈다(사용자 지적). 받는 보조금이 나오는 부처로 바꾼다:
#   mohw2016=보건복지부(기초연금·기초생활·의료·아동), mltmkr=국토교통부(주거급여·K-패스·신혼부부),
#   molab_suda=고용노동부(실업급여·구직수당·고용보험). 실측으로 수집 정상 확인.
REFERENCE_BLOGS = ["mohw2016", "mltmkr", "molab_suda"]
...
규칙:
- **반드시 '개인이 신청해서 현금·바우처·급여·환급·감면을 받는' 대중 복지·지원제도만 뽑아라.**
  그게 아니면 버려라(개수 못 채워도 됨). 육아·청년·주거·에너지·취업·노후·의료 등 대중적 대상.
  신청조건·대상·금액·기간·달라지는 점을 구체 질문형으로.
  예: '부모급여 인상 소급 적용', '에너지바우처 신청 자격', '근로장려금 반기 신청 조건', '주거급여 소득기준'
- **절대 금지(받는 보조금이 아님)**: 문화·영화·전시·공연·축제·박람회·관광·여행·수목원·입장권,
  교통 현황(KTX·SRT 좌석·명당·요금 비교), 음식·레시피, 안전·재해·단속·처벌·수사, 통계·데이터 공개,
  기관 행사·공모전·홍보·이벤트·퀴즈·연예. **과태료·벌금 등 처벌성**, **특정 의약품·주사제 상품명
  (예: ○○주)·희귀질환 치료제 급여** 도 금지(일반인이 검색하는 대표 보조금이 아니다). 나쁜 예:
  '아날로그 영화 뜻', '팔만대장경 디지털화', 'KTX 좌석 늘었나', '아침고요수목원 입장료',
  '하와이 미트전 레시피', '폴라이비주 급여조건', '소방안전관리자 미선임 과태료' — 전부 뽑지 마라.
- **일반 국민 다수가 검색하는 대표 보조금**만(기초연금·아동수당·부모급여·주거급여·청년월세·에너지바우처·
  근로장려금·K패스·아이돌봄 등). 특정 질환·전문직·소수 대상의 세부 급여는 빼라.
- 제목은 계절·명절 접두(‘추석전’·‘명절 전’ 등)나 군더더기를 빼고 검색창에 칠 핵심 질문으로.
  예: '체불임금 추석전 받는법' → '체불임금 받는법'
...
JSON 배열만: [{{"keyword": "...", "source": "어느 정부 소식에서", "why": "왜 검색될지 한 줄"}}]"""
    ...
    raw = raw.strip().removeprefix("```json").removeprefix("```").removesuffix("```").strip()
    # 결정론 backstop — LLM이 흘리는 단속·처벌·통계·데이터류를 키워드 패턴으로 컷
    # ('내가 받는 보조금'이 아닌 것. 2026-09-04 사용자 "일반인이 받는 보조금으로")
    _BLOCK = ("처벌", "징역", "단속", "수사", "범죄", "검거", "압수", "몇명", "명 참석",
              "현황", "통계", "공개목록", "빅데이터", "행사", "박람회", "수상", "위촉",
              # 2026-09-08 부처 소스 교체 후 남는 비수급 잡음 컷 (문화·교통·안전·이벤트)
              "영화", "대장경", "문화재", "전시", "공연", "연극", "관람", "입장권", "입장료",
              "예매", "축제", "이벤트", "방탈출", "퀴즈", "관광", "여행", "수목원", "레시피",
              "KTX", "SRT", "열차", "기차", "좌석", "명당", "공모", "중대재해", "산업안전",
              "폭염", "급성중독", "홍보", "캠페인",
              # 처벌성·특정 의약품 상품명(희귀 항암제 등) — 받는 보조금 아님 (2026-09-08)
              "과태료", "벌금", "과징금", "미선임", "위반", "행정처분",
              "폴라이비주", "블린사이토")
    out, seen = [], set()
    for item in json.loads(raw):
        kw = str(item.get("keyword", "")).strip()
        if len(kw) >= 5 and kw not in seen and not any(b in kw for b in _BLOCK):
            seen.add(kw)
...
_LABOR_MARKERS = ("퇴직연금", "퇴직금", "4대보험", "주휴수당", "하도급", "근로계약",
                  "부당해고", "해고", "수습", "체불", "산재", "중도인출", "원천",
                  "최저임금", "연차수당", "주52시간", "고용보험", "노동청", "근로감독")
