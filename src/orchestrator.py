...
관리 관점 (사용자 확정 2026-08-22): "무조건 쌍으로"라는 기계적 규칙이 아니라,
각 지적의 인과를 추론해 필요한 지시만 낸다 — 두 축이 서로 독립이다:
  ① 이미 발행된 글에 결함이 남아 있는가 → 있으면 교정(fix_post)
  ② 그 결함을 만든 생산 경로가 아직 그대로인가 → 그대로면 예방(prevent/manual),
     이미 파이프라인이 고쳐졌으면 예방 불요
각주 결함이 교정+예방 둘 다였던 건(사용자 지적: 나간 글에도 있고 다음 글도 그렇게 나옴)
두 축이 동시에 참이었기 때문 — 규칙이 아니라 추론의 결과다.
...
## 관리 판단 — 기계적 규칙이 아니라 능동적 추론
각 지적마다 아래 두 축을 **독립으로** 따져, 실제로 필요한 지시만 내려라
(둘 다 참이면 둘 다, 하나만 참이면 하나, 둘 다 아니면 지시 없음):
  ① 이미 발행된 글에 이 결함이 남아 있는가? → 있으면 fix_post로 교정 (post_id 지정)
  ② 이 결함을 만든 생산 경로(작문·이미지·발행 코드)가 아직 그대로인가? → 그대로면
     prevent(정책 힌트로 다음 글 교정) 또는 manual(코드 수정 지시서)로 예방.
     이행 이력에 그 경로가 이미 고쳐졌다고 나오면 예방은 불필요 — 교정만
근거 없이 한쪽 축을 빠뜨리지도 말고, 규칙을 채우려 불필요한 지시를 지어내지도 마라 —
지시의 수가 아니라 인과가 판단 기준이다.
